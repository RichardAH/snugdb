...
private:
  ...
  std::mutex
      big_file_mutex; // locked when incrementing the "next new block" pointer
...
#define OFFSET(byte0, byte1, byte2)                                            \
  (((((uint64_t)(byte0 & 0xFFU)) << 12) + (((uint64_t)(byte1 & 0xFFU)) << 4) + \
    ((uint64_t)(byte2 & 0xFU)))                                                \
   << 18)
...
#define WRITE_KEY(x /* dst */, y /* src */, flags)                             \
  ...
  uint64_t write_big_entry_internal(uint8_t *data, ssize_t len,
                                    uint64_t next_block) {
  ...
  // 1 = bucket full
  // 2 = big blocks full
  int write_entry_internal(uint8_t *data, uint8_t *key, uint8_t *val,
                           uint32_t len) {
    // find the entry
    ...
    for (int i = 0; i < 256 * 1024; i += 1024) {
      ...
      // special edge case: the key doesn't exist and they're trying to delete
      // it
      if (!found && len == 0)
        return 0;
  ...
  int read_entry_internal(uint8_t *data, uint8_t *key, uint8_t *val_out,
                          uint64_t *out_len) {
  ...
  void setup() {
    ...
    if (stat(path.c_str(), &path_stat) != 0)
      throw std::runtime_error("Error checking path: " + path + " - " +
    ...
    // create and map snug.big overflow file
    {
      ...
      if (check_file(new_file.c_str(), BIGSIZE) != 0) {
        ...
        if (result != 0)
          throw std::runtime_error("Failed to create initial file: " +
                                   new_file);
      }
...
public:
  SnugDB(std::string path_) : path(path_) { setup(); }
  ...
  int write_entry(uint8_t *key, uint8_t *val, ssize_t len) {
    ...
    // All existing files are full, allocate a new one
    {
      ...
      void *mapped = mmap(nullptr, file_stat.st_size, PROT_READ | PROT_WRITE,
                          MAP_SHARED, fd, 0);
      close(fd); // Can close fd after mmap
  ...
  int read_entry(uint8_t *key, uint8_t *val_out, uint64_t *out_len_orig) {
    ...
    for (size_t i = 0; i < mapped_files_count; ++i) {
      ...
      int result = read_entry_internal(mapped_files[i], key, val_out, &out_len);
  ...
  void visit_all(void (*f)(uint8_t *, uint8_t *, uint64_t,
    ...
    // if we find 256 entries there then we go to snug.1 and so on until we run
    // out we merge sort the entries into a list for the visit
    ...
    for (uint64_t bucket = 0; bucket < BUCKET_COUNT; ++bucket) {
      ...
      std::vector<uint8_t *> entries;
      {
        ...
        for (int i = 0; i < mapped_files_count; ++i) {
          ...
          for (int entry_count = 0; !IS_ZERO_ENTRY(ptr) && entry_count < 256;
